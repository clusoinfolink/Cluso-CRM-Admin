...
export function VerificationEmailModal({
  ...
  useEffect(() => {
    if (open && view === "compose") {
      ...
      setComposeSubject(serviceName ? `Verification Request – ${serviceName}${candidateName ? ` – ${candidateName}` : ""}` : `Verification Request${candidateName ? ` – ${candidateName}` : ""}`);
    }
  }, [open, view, candidateEmail, candidateName, serviceName]);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: "#fff", borderRadius: "16px", width: "min(95vw, 820px)", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 25px 60px rgba(0,0,0,0.2)", overflow: "hidden" }}>
        {/* Header */}
